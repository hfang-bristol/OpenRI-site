...
Most available interactions are generated or curated between genes (or proteins), and regulatory interactions involving non-coding regions of the genome are less studied, both in terms of resources and utilities. The OpenRI resource addresses these needs, including manual curation of gene-directed regulatory interactions across a wide range of contexts (eg organs, tissues, and cell types), and supporting their use for data mining inclusive of non-coding genome.
...
- [Mining - Browse], navigating contexts (eg organs, tissues, and cell types) to enable interactive exploration of their regulatory interactions; 

- [Mining - RI2Genes], mining the resource to identify potential regulatory genes (see [Example Output](/app/examples/_tmp_RMD_RI2Genes.html)) from user-input data;

- [Mining - RI2Terms], mining the resource to identify potential regulatory genes and enriched ontology terms (see [Example Output](/app/examples/_tmp_RMD_RI2Terms.html)) from user-input data.
...
The OpenRI web-server is developed to be mobile-first and responsive (`frontend`), while a next-generation Perl web framework `backend` enables real-time browsing and mining of the OpenRI resource.
...
```{r search, fig.cap="Faceted search enables centralised data-mining opportunities of the OpenRI resource.", fig.width=10, fig.height=4.3, echo=FALSE, eval=T, fig.align="center",out.width="100%"}
library(png)
library(grid)
...
```{r browse, fig.cap="The context browse interfaces for fetching regulatory interactions via navigating contexts, illustrated for the context `C0001: A549_Roadmap (PMID33828297)`. The integers in the red hexagons denote sequential clicks. `Top`: the browse page lists contexts for regulatory interactions with different modals, including the modal `Activity-By-Contact (ABC)`. `Bottom-left`: the context-specific page for the context `C0001: A549_Roadmap (PMID33828297)`, which lists its regulatory interactions between genomic regions and genes (each gene hyperlinked to the gene-specific page). `Bottom-right`: the gene-specific page for the gene `ROCK1P1`, which lists its regulatory interactions separately for different modals (this is, `ABC`, `PCHiC` and `QTL`), with each context hyperlinked to the context-specific page.", fig.width=10, fig.height=8.8, echo=FALSE, eval=TRUE, fig.align="center",out.width="100%"}
library(png)
library(grid)
...
- `Step 1` and `Step 2`: Enter a list of user-input genomic regions, and specify the reference genome build. Please make sure that the input genomic coordinates (`Step 1`) are consistent with the specified genome build (`Step 2`).

- `Step 3`: Select from a list of contexts under different modals (PCHiC, ABC, and QTL).
...
```{r RI2Genes-results-input, fig.cap="A summary table of the user-input data under the tab `Input Information` returned for the exploration.", fig.width=10, fig.height=6.5, echo=FALSE, eval=TRUE, fig.align="center",out.width="95%"}
library(png)
library(grid)
...
- [Example Output](/app/examples/_tmp_RMD_RI2Terms.html) includes interactive tables for regulatory genes (and evidence) and ontology terms, as well as graphical plots for enriched terms. A summary of input data and the runtime (computed on the server side) is also returned to the users.
...
```{r RI2Terms-results-terms, fig.cap="Ontology Term table lists enriched ontology terms under the tab `Output: Ontology Terms`.", fig.width=10, fig.height=9.2, echo=FALSE, eval=TRUE, fig.align="center",out.width="95%"}
library(png)
library(grid)
...
- Under the tab `Output: Plots`, two output plots `Dotplot` and `Forest plot` illustrate enriched ontology terms. Also provided are the downloadable PDF files.
